...
## Visualization of CA
In addition to Association Plots "APL" produces also other forms of the output. For instance, we can use "APL" to generate a two- and three-dimensional projection of the data in the correspondence analysis space. The so-called biplot visualizes then both cells and genes from the input data. To generate such biplots a `cacomp` object is required. We can convert `r CRANpkg("Seurat")` or `r Biocpkg("SingleCellExperiment")` objects, which have the CA results stored, to a `cacomp` object using the function `as.cacomp()`:
