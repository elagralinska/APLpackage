...
# Preprocessing
## Setup
For this vignette we will use as an example the 3k Peripheral Blood Mononuclear Cell (PBMC) data from 10x Genomics.
...
but allows to represent both cells and genes in the same space. 
...
The function `cacomp` takes as input any non-negative count matrix, be it a single-cell RNA-seq, bulk RNA-seq or other data. If we would like to perform feature selection before CA, we can set the argument `top` to the desired number of genes to keep based on the variance of the chi-square components matrix. 
...
The above plots let us interactively explore the CA space. If a static version is preferred we can obtain it by setting `type = "ggplot"`. This option is available for all plotting functions with interactive plots (with the exception of the 3D plot).
...
In the scree plot above we can see that the first dimension explains only ~0.75% of the inertia in the data and the the bend in the scree plot is at roughly 10 dimensions. The first dimensions however explain only a small part of the total inertia. 
...
apl(ca,
    ...
    type = "ggplot")
```
Cells that are a part of the chosen cell cluster are colored in red, all other cells are colored in green. Genes are represented by blue circles.

The further to the right a gene is located the more associated it is with the chosen cluster of cells. Additionally, the lower the y-axis value, the more specific it is for the selected cluster. As we can see in the plot, known platelet marker genes are located in the right partof the plot. If we are interested in not only the genes associated with a selected cluster of cells, but also other cells, we can additionally plot the cells in the same plot through the argument `show_cols = TRUE`. This way we can identify other clusters that are highly similar or cells that should be merged with the chose condition.

The Salpha score encompasses both of the above mentioned aspects of condition specificity and allows us to rank genes by how relevant they are for a condition/cluster:
...
In the top 6 most highly ranked genes we can find well-known platelet marker genes such as GP9 (Glycoprotein IX Platelet) or ITGA2B (Integrin Subunit Alpha 2b).
We can also plot these genes by using functions provided in `r CRANpkg("Seurat")`:
...
Lets have a closer look at cluster 3. We first calculate the the Association Plot coordinates for both cells and genes, and then the Salpha score for the genes.
...
The GO enrichment analysis clearly suggests that cells in cluster 3 are B cells. We can further look at the number of typical B cell marker genes in the Association Plot.
...
All of these B cell marker genes are clearly highly associated with the cluster 3, which confirms the correct annotation we did in the preprocessing of the data.
